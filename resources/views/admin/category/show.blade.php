...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Страница категории</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
    ...
    @php
        $categoryProducts = $category->products()->paginate($counter_paginate);
    @endphp
    <div class="card">
        <h3 class="h3 p-2 text-center">Товары категории</h3>
        <div class="card-header">
            {{ $categoryProducts->links('pagination.custom') }}
        </div>
        <!-- /.card-header -->
                ...
                @foreach ($categoryProducts as $product)
                    <tr>
                        <td class="align-middle">
                            @if($categoryProducts->lastPage() === $categoryProducts->currentPage())
                                {{-- Пагинация для последней страницы --}}
                                @php $first_el = $categoryProducts->lastItem() - $categoryProducts->count(); @endphp
                                {{ $first_el + $loop->iteration}}.
                            @else
                                {{-- Пагинация для остальных страниц --}}
                                {{ $categoryProducts->count() * ($categoryProducts->currentPage() - 1) + $loop->iteration  }}.
                            @endif
                        </td>
                        <td class="align-middle">
